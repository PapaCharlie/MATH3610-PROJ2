...
fifteen_minutes = 15 * ticks_per_minute;
...
ideal_quadrant_size = speed * fifteen_minutes;
ideal_quadrant_size = ideal_quadrant_size - mod(ideal_quadrant_size, 2);
...
true_quadrant_size = true_quadrant_size + (length(divs(mod(divs,2) == 0)) == 1)
divs = divisors(true_quadrant_size);
divs = divs(2:end-1);
...
while tick ~= total_ticks
  ...
  % (tick - last_visited)'
  % waitforbuttonpress
  unwatched_zones_per_tick(tick) = numel(last_visited((tick - last_visited) > fifteen_minutes));
end
