...
\par In conclusion, both the Block Model and the Optimal Path model seem to agree on the fact that one would need at least 50 drones to optimally cover Manhattan/Gotham City. Unfortunately, those drones have infinite fuel, therefore the true solution needs to be adjusted according to the ratio of the drones' flight time to their recharge time; but this can be done without changing the strategy itself, just the timing of the drones' deployment. If we wanted to assuage the fears of some of the troublemakers, then we we would need at least 75 drones to properly cover 90\% of Manhattan. Thankfully, given that this is Gotham City, 50 to 100 drones seems quite cheap.
